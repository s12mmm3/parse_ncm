...
class CacheService:
    ...
    async def _init_video_cache(cls):
        ...
        async with _video_cache_lock:
            if not VIDEO_CACHE_DIR.exists():
                VIDEO_CACHE_DIR.mkdir(parents=True, exist_ok=True)
                logger.info(f"已创建视频缓存目录: {VIDEO_CACHE_DIR}", "网易云解析")
            ...
            if CACHE_INDEX_FILE.exists():
                try:
                    ...
                    logger.error(f"加载视频缓存索引失败: {e}", "网易云解析")
                    _video_cache_index = {}
            ...
            if invalid_keys:
                logger.info(f"已移除 {len(invalid_keys)} 个无效的缓存条目", "网易云解析")
    ...
    async def get_video_cache(cls, video_id: str, page_num: int = 0) -> Optional[Path]:
        ...
        async with _video_cache_lock:
            ...
            if not cache_info:
                logger.debug(f"视频缓存未命中: {cache_key}", "网易云解析")
                return None

            file_path_str = cache_info.get("file_path")
            if not file_path_str:
                logger.warning(f"缓存索引中的文件路径为空: {cache_key}", "网易云解析")
                return None

            file_path = Path(file_path_str)
            if not file_path.exists():
                logger.warning(f"缓存文件不存在: {file_path}", "网易云解析")
            ...
            logger.info(f"视频缓存命中: {cache_key} -> {file_path}", "网易云解析")
            return file_path
    ...
    async def save_video_to_cache(
        ...
        if not file_path.exists():
            logger.warning(f"要缓存的文件不存在: {file_path}", "网易云解析")
            return False
        ...
        try:
            if str(file_path) == str(cache_file_path):
                logger.debug(f"文件已在缓存目录中，无需复制: {file_path}", "网易云解析")
            else:
                ...
                logger.debug(
                    f"已复制文件到缓存: {file_path} -> {cache_file_path}", "网易云解析"
                )
            ...
            logger.info(
                f"视频已保存到缓存: {cache_key} -> {cache_file_path}", "网易云解析"
            )
            return True
        except Exception as e:
            logger.error(f"保存视频到缓存失败: {e}", "网易云解析")
            return False
    ...
    async def should_parse_url(cls, url: str, session: EventSession) -> bool:
        ...
        if context_key not in _url_context_caches:
            ...
            logger.debug(f"为上下文 '{context_key}' 创建了新的缓存", "网易云解析")
        ...
        if timestamp is None:
            logger.debug(
                f"URL '{url}' 在上下文 '{context_key}' 的缓存中未找到", "网易云解析"
            )
            ...
            if current_time - timestamp > cache_ttl_seconds:
                ...
                logger.debug(
                    f"URL '{url}' 在上下文 '{context_key}' 的缓存未过期", "网易云解析"
                )
                context_cache.move_to_end(url)
                return False
    ...
    async def add_url_to_cache(cls, url: str, session: EventSession):
        ...
        context_cache[url] = current_time
        logger.debug(
            f"手动添加/更新 URL '{url}' 到上下文 '{context_key}' 的缓存", "网易云解析"
        )
    ...
    async def clear_url_cache(cls, context_key: Optional[str] = None):
        ...
        if context_key:
            if context_key in _url_context_caches:
                ...
                logger.info(f"已清空上下文 '{context_key}' 的缓存", "网易云解析")
            else:
                logger.info(f"尝试清空缓存，但未找到上下文 '{context_key}'", "网易云解析")
        else:
            _url_context_caches.clear()
            logger.info("已清空所有上下文的缓存", "网易云解析")
    ...
    async def clean_expired_cache(cls, force: bool = False) -> int:
        ...
        async with _clean_lock:
            ...
            if total_cleaned > 0:
                logger.info(f"缓存清理完成，共清理 {total_cleaned} 个文件", "网易云解析")
    ...
    async def _clean_video_cache(cls, force: bool = False) -> int:
        ...
        if expiry_days <= 0 and not force:
            logger.debug("视频缓存过期时间设置为0或负数，跳过清理", "网易云解析")
            return 0
        ...
        async with _video_cache_lock:
            ...
            for cache_key, cache_info in to_clean:
                ...
                if file_path_str:
                    ...
                    if file_path.exists():
                        try:
                            file_path.unlink()
                            logger.debug(f"已删除缓存文件: {file_path}", "网易云解析")
                            cleaned_count += 1
                        except Exception as e:
                            logger.warning(
                                f"删除缓存文件失败: {file_path}, 错误: {e}", "网易云解析"
                            )
            ...
            if cleaned_count > 0:
                logger.info(
                    f"视频缓存清理完成，共清理 {cleaned_count} 个文件", "网易云解析"
                )
            ...
            for file in files:
                try:
                    ...
                    if file_age_seconds > expiry_seconds:
                        ...
                        file.unlink()
                        cleaned_count += 1
                        logger.debug(f"已清理过期文件: {file.name}", "网易云解析")
                except Exception as e:
                    logger.warning(f"清理文件 {file.name} 时出错: {e}", "网易云解析")
    ...
    async def _auto_clean_task(cls):
        ...
        logger.info(
            f"缓存自动清理任务启动，间隔: {clean_interval_hours} 小时", "网易云解析"
        )
        ...
        while True:
            try:
                ...
                logger.error(f"缓存自动清理任务出错: {e}", "网易云解析")
    ...
    def _get_context_key(session: EventSession) -> str:
        ...
        if session.id3:
            ...
            logger.warning(
                "无法从 Session 中获取有效的上下文 ID，将使用全局缓存 Key", "网易云解析"
            )
    ...
    async def _save_video_cache_index(cls):
        ...
        try:
            ...
            logger.debug(
                f"视频缓存索引已保存: {len(_video_cache_index)} 个条目", "网易云解析"
            )
        except Exception as e:
            logger.error(f"保存视频缓存索引失败: {e}", "网易云解析")
    ...
    async def _load_url_cache_from_disk(cls):
        ...
        if not URL_CACHE_FILE.exists():
            logger.info("URL缓存文件不存在，将使用空缓存", "网易云解析")
            return
        ...
        try:
            ...
            logger.error(f"从磁盘加载URL缓存失败: {e}", "网易云解析")
            _url_context_caches = {}
    ...
    async def _save_url_cache_to_disk(cls):
        ...
        try:
            async with _url_cache_lock:
                ...
                logger.debug(f"URL缓存数据已保存到磁盘: {URL_CACHE_FILE}", "网易云解析")
        except Exception as e:
            logger.error(f"保存URL缓存到磁盘失败: {e}", "网易云解析")
